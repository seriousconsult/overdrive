"""VirtualBox serial console attach for the Kali test client."""
...
def serial_console_instructions(vboxmanage: str, endpoint: str) -> str:
    ...
    return (
        ...
        f"  rm -f {CLIENTK_SERIAL_PTY_LINK_PATH}\n"
        f"  socat -d -d UNIX-CONNECT:{endpoint} PTY,link={CLIENTK_SERIAL_PTY_LINK_PATH},raw,echo=0\n"
        f"  screen {CLIENTK_SERIAL_PTY_LINK_PATH} {SERIAL_BAUD}\n"
    )
...
def nudge_kali_boot_menu(*, rounds: int = 8, interval_s: float = 1.0) -> None:
    """Send Enter a few times on COM1 in case grub is still waiting."""
    print("[overdrive] Nudging Kali serial boot (Enter) for unattended start...")
    deadline = time.monotonic() + 20.0
    last_err: OSError | None = None
    while time.monotonic() < deadline:
        for host in serial_tcp_host_candidates(SERIAL_TCP_HOST):
            try:
                with socket.create_connection((host, KALI_SERIAL_TCP_PORT), timeout=2.0) as sock:
                    sock.settimeout(0.5)
                ...
                last_err = exc
        time.sleep(0.5)
    print(f"[!] Could not nudge Kali serial boot ({last_err}); relying on grub serial console.")
