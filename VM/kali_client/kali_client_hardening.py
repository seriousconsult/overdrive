"""Test client hardening scripts and systemd assets."""
...
CLIENT_FIREWALL_SERVICE = """[Unit]
Description=Test client firewall
DefaultDependencies=no
Before=network-pre.target network.target lab-net-up.service
After=local-fs.target

[Service]
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/client-firewall start
ExecStop=/usr/local/sbin/client-firewall stop

[Install]
WantedBy=multi-user.target
"""

CLIENT_FIREWALL_SCRIPT = r"""#!/bin/bash
...
CLIENT_HARDENING_SCRIPT = r"""#!/bin/bash
# Security hardening for the disposable Kali test client VM.
set -eu
...
remove_if_installed() {
  ...
  if dpkg-query -W -f='${Status}' "$pkg" 2>/dev/null | grep -q 'install ok installed'; then
    DEBIAN_FRONTEND=noninteractive apt-get purge -y "$pkg" >/dev/null 2>&1 || \
      DEBIAN_FRONTEND=noninteractive apt-get remove -y --purge "$pkg" >/dev/null 2>&1 || true
  fi
}
...
SYSCTL
  ...
  cat > /etc/systemd/system/overdrive-sysctl-hardening.service <<'UNIT'
[Unit]
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/bin/bash -c 'if command -v sysctl >/dev/null 2>&1 && [ -f /etc/sysctl.d/99-overdrive-client-hardening.conf ]; then while IFS= read -r line; do case "$line" in ""|#*) continue ;; esac; setting="$(printf "%s" "$line" | sed "s/[[:space:]]*=[[:space:]]*/=/")"; sysctl -w "$setting" >/dev/null 2>&1 || true; done < /etc/sysctl.d/99-overdrive-client-hardening.conf; fi'

[Install]
WantedBy=multi-user.target
UNIT
  systemctl daemon-reload
  systemctl enable overdrive-sysctl-hardening.service
}
...
install_filesystem_hardening() {
  ...
  rm -f /root/.wget-hsts /root/.python_history /root/.lesshst 2>/dev/null || true
}
...
clean_private_artifacts() {
  ...
  rm -f /root/.bash_history /root/.wget-hsts /root/.python_history /root/.lesshst 2>/dev/null || true
...
remove_remote_login_artifacts() {
  for svc in ssh sshd ssh.service sshd.service dropbear dropbear.service \
    cloud-init cloud-init-local cloud-config cloud-final \
    NetworkManager NetworkManager-wait-online telnetd vsftpd nginx apache2 cron crond; do
    disable_service "$svc"
  done
  rm -f \
    /usr/sbin/sshd \
...
remove_matching_packages cloud-init
...
GUARD
  ...
  cat > /etc/systemd/system/overdrive-no-remote-services.service <<'UNIT'
[Unit]
Description=Disable remote login daemons on the disposable lab client
DefaultDependencies=no
Before=network-pre.target ssh.service sshd.service dropbear.service cloud-init.service
After=local-fs.target

[Service]
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/overdrive-no-remote-services
...
UNIT
  ...
  systemctl daemon-reload
  systemctl enable overdrive-no-remote-services.service
}
...
for pkg in openssh-server openssh-client openssh-sftp-server dropbear telnet; do
...
rm -rf /etc/ssh /etc/dropbear 2>/dev/null || true

systemctl enable client-firewall.service
"""
