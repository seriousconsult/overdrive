...
Serial console named pipe:
  ...
  2. The VirtualBox named pipe is the "plastic tube" attached to that serial port. One end is
     connected to the VM; the other end appears on the Windows host as
     ``\\.\pipe\OpenWrt_LAN_Client_serial``.
  3. PuTTY is the "walkie-talkie". Set PuTTY Connection type to Serial, Serial line to
     ``\\.\pipe\OpenWrt_LAN_Client_serial``, and Speed to ``115200``. Anything typed there goes
     into the VM serial login; anything the VM writes to the serial console comes back to PuTTY.
...
SERIAL_PIPE_NAME = r"\\.\pipe\OpenWrt_LAN_Client_serial"
SERIAL_BAUD = "115200"
...
def configure_serial_named_pipe(vboxmanage: str) -> None:
    """Expose COM1 as a Windows named pipe that PuTTY can attach to."""
    print(
        f"Serial console: COM1 -> named pipe {SERIAL_PIPE_NAME} "
