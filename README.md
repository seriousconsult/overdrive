...
- The Alpine LAN client scrubs tracking identifiers at build time (neutral hostname `desktop`, fresh Dell NIC MAC, tame DHCP client identity, cleared `machine-id`, generic timezone User-Agent). Rebuild the Alpine VM after changing those settings. LAN silence is still expected and discovery probes may still score it as lab-like.
