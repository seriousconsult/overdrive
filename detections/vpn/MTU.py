#!/usr/bin/env python3
"""Local link MTU observation (not path MTU to a remote host).

Purpose: Infer encapsulation / tunnel overhead from the smallest MTU among visible interfaces.

VPNs often reduce effective MTU on tunnel interfaces; many commercial VPNs still show 1500 on the
default route from this namespace — weak signal alone.

Score (1–5): 1 = typical Ethernet-sized MTU seen locally. 5 = very small MTU (heavy
encapsulation). Middle scores = gradual reduction.

Environment: Linux / WSL with ``ip -o link``. WSL2 may hide host VPN interfaces.
...
def main() -> int:
    return _run_mtu_analysis()


def _run_mtu_analysis() -> int:
    mtu, tunnel_ifaces = get_link_mtu_info()
    score = calculate_mtu_score(mtu)
    on_wsl = False
    try:
        on_wsl = "microsoft" in (
            open("/proc/version", "r", encoding="utf-8", errors="ignore").read().lower()
        )
    except OSError:
    ...
    if on_wsl:
        print(
            "Note: WSL2 networking can hide host VPN/tunnel interfaces from the Linux namespace; "
            "treat MTU as a weak signal."
        )
    print("-" * 30)
    print(f"SCORE: {score}")
    ...
    if score == 1:
        status = (
            "Standard local link MTU (often 1500 on Ethernet). This does not prove you are not "
            "using a VPN; many VPNs do not show up as a reduced MTU on the interface this script "
            "can see, and WSL can obscure host tunnels."
        )
    elif score == 2:
        status = (
            "Slightly reduced packet size. Likely local virtualization (WSL/VM) or light overhead; "
            "still not definitive for VPNs."
        )
    elif score == 3:
        status = (
            "Moderate packet reduction. Suggestive of a tunnel, VPN, or custom network stack, "
            "but not definitive alone."
        )
    else:
        status = (
            "Small packet size detected. Strong signal of encapsulation/tunneling on at least one "
            "visible interface, but not a full attribution."
        )
