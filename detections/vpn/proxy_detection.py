...
from detections.common.common_config import HTTP_ECHO_URLS, PROXY_TIMEOUT, USER_AGENTS
from detections.common.common_utils import env_proxy_info

UA = {"User-Agent": USER_AGENTS["proxy"]}
...
def _fetch_echo_headers() -> tuple[dict[str, str] | None, str | None]:
    ...
    for url in HTTP_ECHO_URLS:
        try:
            r = requests.get(url, headers=UA, timeout=PROXY_TIMEOUT)
            r.raise_for_status()
            data = r.json()
...
def check_proxy_headers() -> tuple[int, str]:
    env_on, env_lines = env_proxy_info()
