...
# binary applies when you run ./NAT_OS.py from detections/router/.
"""
...
Host-authenticity score: 1 = residential/NAT evidence, 3 = ambiguous,
...
def compute_nat_score(stats: dict[str, dict[str, set]]) -> tuple[int, str]:
    """
    1 - Strong NAT/router household signal.
    2 - Some multi-host or normal single-stack traffic evidence.
    3 - Too little data or borderline ambiguity.
    4/5 are reserved for artificial-host evidence, which this probe does not produce.
    """
    if not stats:
        return 3, "No IPv4 packets observed; cannot confirm home NAT behavior."
    ...
    for src_ip, data in stats.items():
        ...
        if has_linux and has_windows:
            return (
                1,
                f"Residential NAT pattern: {src_ip} shows Linux- and Windows-range TTLs in one source ({sorted(ttls)[:12]}...).",
            )
        if dttl >= 4:
            best = min(best, 1)
            best_note = f"{src_ip} shows {dttl} distinct TTL values (multi-host/NAT-like)."
        elif dttl >= 3:
            best = min(best, 2)
            best_note = f"{src_ip} shows {dttl} distinct TTL values (some home-network evidence)."
        elif dttl >= 2:
            best = min(best, 2)
