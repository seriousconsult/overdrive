...
VPNs wrap your data in an "envelope" (encapsulation). This makes the data packets slightly smaller than normal internet packets.
Standard Packet: 1500 bytes.
VPN Packet: Usually 1300–1450 bytes.
If a website sees you sending full 1500-byte packets, it knows you are not using a VPN, or your VPN is misconfigured.
'''
...
def get_mtu():
    try:
        ...
        for i, mtu in enumerate(mtus):
            mtu_val = int(mtu)
            status = "Standard (Likely no VPN)" if mtu_val == 1500 else "Reduced (Possible VPN/Tunnel)"
