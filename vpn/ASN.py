...
which Autonomous System Number (ASN) you are coming from.
Residential ASN: Owned by companies like Comcast, AT&T, or Verizon. 
These are more trusted because they represent real people in houses.
Data Center/Hosting ASN: Owned by companies like DigitalOcean, Amazon (AWS), or M247.
Most VPN providers host their servers in well known data centers. If a website performs an ASN lookup and sees 
that your IP belongs to a Data Center instead of a Residential ISP, it assumes you may be using a VPN,
 Proxy, or Bot.
...
def lookup_asn(ip_address=""):
    ...
    try:
        ...
        if "M247" in org or "Datacamp" in org or "Hosting" in org:
            print("🚨 ALERT: Status: Likely a VPN/Data Center IP")
        else:
